...
library(ggsidekick)
theme_set(theme_sleek(base_size = 10))
...
sort(unique(lb$fishery))
# FIXME
# Split fishery in more detail based on focatnat, update the rest of the script also
# See code in ecosystem figures

# "Räkfiske i Skagerrak, Kattegatt och Nordsjön" into:
# 1) Räkfiske med rist/tunnel i Skagerrak, Kattegatt och Nordsjön (OTB_CRU_32-69_0_0)
# 2) Räkfiske med rist i Skagerrak, Kattegatt och Nordsjön (OTB_CRU_32-69_2_22)

#   "Bottentrålfiske efter kräfta och fisk i Skagerrak och Kattegatt"
# 1) Bottentrålfiske efter kräfta och fisk i Skagerrak och Kattegatt
# 2) Bottentrålfiske efter kräfta med rist [i Skagerrak och Kattegatt] (OTB_CRU_70-89_2_35, OTB_CRU_70-89_2_35_0_0)

# Bottentrålfiske efter kräfta och fisk i Nordsjön; remove "kräfta" from name? To match Daniel
# Fiske med passiva redskap i Skagerrak och Kattegatt; call it Fiske med passiva redskap på västkusten to match Daniel?

# FIXME: OTB_CRU_32-69_2_22 och OTB_CRU_32-69_0_0 ska gå till varsinn kategori i "Räkfiske i Skagerrak, Kattegatt och Nordsjön"
...
# 
# lb |>
#   filter(fishery == "Räkfiske i Skagerrak, Kattegatt och Nordsjön") |>
#   distinct(REDSKKOD)
# 
# unique(lb$fishery)
# 
# lb |>
#   filter(fishery == "Fiske efter siklöja med bottentrål") |>
#   distinct(REDSKKOD)
# 
# lb |>
#   filter(fishery == "Fiske efter siklöja med bottentrål") |>
...
# 
# lb |>
#   filter(fishery == "Bottentrålfiske efter kräfta och fisk i Skagerrak och Kattegatt") |>
#   distinct(foCatNat)
# 
# lb |>
#   filter(fishery == "Bottentrålfiske efter kräfta och fisk i Skagerrak och Kattegatt") |>
#   filter(foCatNat == "OTB_CRU_32-69_2_22")
# 
# lb |>
#   filter(fishery == "Bottentrålfiske efter kräfta och fisk i Skagerrak och Kattegatt") |>
#   filter(foCatNat == "OTB_CRU_32-69_0_0")
# 
# lb |>
#   filter(fishery == "Fiske i Öresund") |>
#   distinct(foCatNat)
# 
# lb |>
#   filter(fishery == "Bottentrålfiske efter kräfta och fisk i Skagerrak och Kattegatt") |>
#   filter(foCatNat == "OTB_DEF")

# sort(unique(lb$foCatNat))
...
lb <- lb |>
  mutate(
    year = year(as.Date(as.character(AVRESDAT), format = "%Y%m%d")),
...
lb <- lb |>
  ...
  filter(KVANT > 0) |>
  filter(!FNAMNSVE %in% c(
    "Redskap Med Sälskada (St Redskap)",
    "Obest. benfiskar"
  )) |>
  drop_na(FNAMNSVE)
...
landings_map_select <- plot_map_m +
  ...
  scale_fill_viridis(
    trans = "sqrt", option = "plasma",
    na.value = "yellow", name = "Landningar\n(tusen ton)",
  ...
  coord_sf(ylim = c(20, 78))

ggsave(plot = landings_map_select, paste0(home, "/figures/entire_fleet/landings_map_select.png"), width = 17, height = 13, units = "cm")
```
...
p1 <- land_plot +
  geom_point(
    data = land_quant_old, aes(landhamn_lon, landhamn_lat, color = kvant_sum, size = kvant_sum),
    alpha = 0.4
  ) +
  annotate("text",
    label = paste0("Medel = ", round(mean(land_quant_old$kvant_sum)), " tusen  ton"),
...
p2 <- land_plot +
  geom_point(
    data = land_quant_new, aes(landhamn_lon, landhamn_lat, color = kvant_sum, size = kvant_sum),
    alpha = 0.4
  ) +
  annotate("text",
    label = paste0("Medel = ", round(mean(land_quant_new$kvant_sum)), " tusen ton"),
    x = -Inf, y = Inf, hjust = -0.06, vjust = 2, color = "gray20", size = 2
  ) +
  annotate("text",
    label = "År 2016-2022",
...
p1 <- plot_map_m +
  geom_curve(
    data = link |> filter(FNAMNSVE == "Skarpsill"),
    aes(
      x = lon, xend = landhamn_lon, y = lat, yend = landhamn_lat,
      color = sum_kvant / 1000
    ),
    curvature = -0.15, angle = 90, alpha = 0.1, linewidth = 0.1
  ) + # color = "grey20") +
  geom_point(
    data = link |> filter(FNAMNSVE == "Skarpsill"),
  ...
  coord_sf(
    xlim = c(
      min(c(link$landhamn_lon, link$lon)),
      max(c(link$landhamn_lon, link$lon))
    ),
    ylim = c(
      min(c(link$landhamn_lat, link$lat)),
      max(c(link$landhamn_lat, link$lat))
    )
  ) +
  scale_color_viridis(
    trans = "log10", breaks = c(0.1, 1, 10, 100),
...
p2 <- plot_map_l +
  geom_curve(
    ...
    curvature = -0.15, angle = 90, alpha = 0.1, linewidth = 0.07
  ) + # color = "grey20") +
  geom_point(
    data = link |> filter(FNAMNSVE == "Skarpsill"),
...
p1 <- plot_map_m +
  geom_curve(
    data = link_agg,
    aes(
      x = lon2, xend = landhamn_lon, y = lat2, yend = landhamn_lat,
...
alluvial <- alluvial |>
  ...
  mutate(
    HAVSKOD2 = ifelse(HAVSKOD == "ÖN", "Norra\nÖstersjön", NA),
...
sea_species_landings <-
  ggplot(
    alluvial,
    aes(
      axis1 = FNAMNSVE, axis2 = HAVSKOD2, axis3 = LANDLAND,
      y = sqrt(sum_kvant)
    )
  ) +
...
p2 <- ggplot(cumsum_harb, aes(n, cum_sum_prop, color = factor(year))) +
  ...
  xlim(0, 65) +
  geom_segment(
    data = ogives_harb, aes(x = n, xend = n, y = 0, yend = cum_sum_prop, color = factor(year)),
...
knitr::kable(
  lb |>
    filter(year > 2015) |>
    summarise(kvant_sum = round(sum(KVANT) / 1000000), .by = LANDHAMN) |>
    arrange(desc(kvant_sum)) |>
    rename(Hamn = LANDHAMN, "Totala landingar (tusen ton)" = kvant_sum) |>
    drop_na(Hamn) |> # FIXME Why NA here?!
    slice(1:10),
  format = "html",
  full_width = FALSE,
  caption = "Top 10 landningshamnar (fångstvikt) in 2016-2002"
)
...
knitr::kable(
  lb |>
    ...
    summarise(kvant_sum = sum(KVANT) / 1000000, .by = LANDHAMN) |>
    arrange(desc(kvant_sum)) |>
    rename(Hamn = LANDHAMN, "Totala landingar (tuen ton)" = kvant_sum) |>
    slice(1:10),
  format = "html",
  full_width = FALSE,
  caption = "Top 10 landningshamnar (fångstvikt) in 2016-2002"
...
p1 <- ggplot(lb_vessel, aes(LÄNGD)) +
  ...
  geom_text(
    data = lb_vessel_count, aes(Inf, Inf, label = lab),
...
change <- lb |>
  ...
  mutate(
    LÄNGD_cat = cut(LÄNGD, breaks = seq(0, max(LÄNGD), by = 3)),
  ...
  separate_wider_delim("LÄNGD_cat",
    delim = "-", names = c("min_LÄNGD", "max_LÄNGD"),
    cols_remove = FALSE
  ) |>
...
p2 <- ggplot(
  data = change,
  aes(
    x = reorder(LÄNGD_cat, min_LÄNGD), xend = reorder(LÄNGD_cat, min_LÄNGD),
...
fishery_vessel <- ggplot(lb_fish_ves, aes(`2016-2022`, reorder(fishery, `2016-2022`))) +
  ...
  geom_segment(
    aes(
      xend = `2016-2022`, x = `2003-2015`, y = reorder(fishery, `2016-2022`),
      color = trend
    ),
...
p2 <- ggplot(cumsum, aes(n, cum_sum_prop, color = factor(year))) +
  ...
  xlim(0, 65) +
  geom_segment(
    data = ogives, aes(x = n, xend = n, y = 0, yend = cum_sum_prop, color = factor(year)),
    alpha = 0.9, linetype = 2
  ) +
  scale_color_manual(values = colors, name = "") +
...
p1 <- ggplot(
  lb_gear |> filter(group == ">1% av totala landningarna"),
...
landings_top_12_sp <- ggplot(
  lb_spec |> filter(FNAMNSVE %in% order$FNAMNSVE[1:12]),
...
landings_sp <- ggplot(
  lb_spec_all |> filter(FNAMNSVE %in% lb_spec_all$FNAMNSVE[1:30]),
...
lb_spec_split <- lb_spec |>
  ...
  mutate(sp = ifelse(FNAMNSVE %in% c(
    "Skarpsill", "Tobis", "Sill/Strömming",
    "Makrill", "Siklöja", "Blåvitling"
  ), "Pelagic", "Demersal"))
...
pel <- polar_theme +
  ...
  scale_fill_brewer(palette = "Set3", name = "") +
  scale_y_continuous(
    limits = c(-50, (ymax |> filter(sp == "Pelagic") |> pull(month_sum)) * 1.1),
    expand = c(0, 0)
  ) +
  geom_col(
    data = lb_spec_split |> filter(sp == "Pelagic"),
    aes(x = month_name, y = sum_kvant / 1000000, fill = FNAMNSVE)
  ) +
  # Annotate custom scale inside plot
  annotate(
    x = "December", y = hlines_pel$y[1] * 0.8, label = paste(hlines_pel$y[1], "tusen ton"),
    geom = "text", color = "grey50", size = 3
  ) +
  annotate(
    x = "December", y = hlines_pel$y[2] * 0.9, label = paste(hlines_pel$y[2], "tusen ton"),
    geom = "text", color = "grey50", size = 3
  ) +
  ggtitle("Pelagiska")
...
dem <- polar_theme +
  ...
  scale_fill_brewer(palette = "Set3", name = "") +
  scale_y_continuous(
    limits = c(-5, (ymax |> filter(sp == "Demersal") |> pull(month_sum)) * 1.1),
    expand = c(0, 0)
  ) +
  geom_col(
    data = lb_spec_split |> filter(sp == "Demersal"),
    aes(x = month_name, y = sum_kvant / 1000000, fill = FNAMNSVE)
  ) +
  # Annotate custom scale inside plot
  annotate(
    x = "December", y = hlines_dem$y[1] * 0.8, label = paste(hlines_dem$y[1], "tusen ton"),
    geom = "text", color = "grey50", size = 3
  ) +
  annotate(
    x = "December", y = hlines_dem$y[2] * 0.9, label = paste(hlines_dem$y[2], "tusen ton"),
    geom = "text", color = "grey50", size = 3
  ) +
  ggtitle("Demersala")
...
lb_fish_sp_order <- lb_fish_sp |>
  summarise(sum_kvant_tot = sum(sum_kvant), .by = fishery) |>
  arrange(sum_kvant_tot)

# Facet... 
lb_fish_sp <- lb_fish_sp |>
  mutate(group = ifelse(fishery %in% c("Pelagiskt fiske med aktiva redskap",
                                       "Bottentrålfiske efter torsk i Östersjön"),
                        1, 2))

# FIXME: What is goiiiiing on with with sqrt-scales?!
fish_sp <- ggplot(
  lb_fish_sp,
  aes(sum_kvant, factor(fishery, levels = lb_fish_sp_order$fishery), fill = species)) +
  geom_bar(stat = "identity") +
...
discard_all_plot <- ggplot(
  discard_sum |> filter(!year %in% c(2020, 2021)),
...
ggsave(plot = discard_sp, "figures/entire_fleet/discard_sp.png", width = 17, height = 14, units = "cm")
```
...
discard_ratio_heat <- ggplot(discard_trends, aes(year, reorder(Art, prop), fill = prop)) +
  ...
  geom_text(
    data = md |> filter(fishery == head(fishery, 1)), size = 3.5,
...
for (i in unique(lb_clean$fishery)) {
  ...
  ggplot(
    lb_spec_sub |> filter(FNAMNSVE %in% order_sub$FNAMNSVE[1:12]),
    aes(year, sum_kvant / 1000000, fill = FNAMNSVE)
  ) +
    # geom_bar(stat = "identity") +
    geom_area() +
  ...
  hlines <- tibble(y = seq(0, ymax, length.out = 4)) |>
    round(digits = -2) |>
    filter(!y == min(y))
  ...
  polar_theme +
    ...
    geom_col(
      data = lb_spec_month_sub,
      aes(x = month_name, y = sum_kvant / 1000, fill = FNAMNSVE), alpha = .95
    ) +
    # Annotate custom scale inside plot
    annotate(
      x = "December", y = hlines$y[1] * 0.8, label = paste(hlines$y[1], "ton"),
      geom = "text", color = "grey50", size = 3
    ) +
    annotate(
      x = "December", y = hlines$y[2] * 0.9, label = paste(hlines$y[2], "ton"),
      geom = "text", color = "grey50", size = 3
    ) +
    annotate(
      x = "December", y = hlines$y[3] * 0.93, label = paste(hlines$y[3], "ton"),
      geom = "text", color = "grey50", size = 3
    ) +
    ggtitle(i)
  ...
  lb_gear_sub <- lb_sub |>
    drop_na(Redskap2) |>
    summarise(sum_kvant = sum(KVANT), .by = c(year, Redskap2)) |> 
    # Filter gears with more than 1%.. 
    mutate(sum_gear = sum(sum_kvant), .by = Redskap2) |>
    mutate(sum = sum(sum_kvant),
           percent = (sum_gear / sum)*100) |> 
    filter(percent > 1)
    

  ggplot(lb_gear_sub, aes(year, sum_kvant / 1000, fill = Redskap2)) +
    geom_area() +
    # facet_wrap(~Redskap2, scales = "free_y") +
    # geom_bar(stat = "identity") +
    scale_fill_manual(values = pal_long, name = "") +
    theme_sleek(base_size = 9) +
    theme(
  ...
  ggplot(discard_sub, aes(year, discards, fill = Art)) +
    geom_bar(stat = "identity") +
    geom_text(
      data = md |> filter(fishery == i), size = 2.5,
      aes(year, y, label = label), inherit.aes = FALSE, angle = 90,
      color = "grey50", hjust = -0.05
    ) +
