...
# Instructions:
# Every year I rerun this script. Get new PreCl data from Sofia and update the
# file path in the "Load data" section below to point to the new data.
# The output folder (output/<year>/HaV) is derived automatically from the max
# fishing date in the loaded data -- see the "Define output path" section below.
#
# Instead of clicking Render, run render() in the console -- it renders the
# document and moves the resulting HTML into the output folder (output/<year>/HaV,
# same folder the .xlsx table is written to). The script is rerun the year after
# the data's last complete year, so the current calendar year is used.
render <- function(output_path = NULL) {
  quarto::quarto_render(here::here("hav-fishery-figures.qmd"))
  if (is.null(output_path)) output_path <- here::here("output", format(Sys.Date(), "%Y"), "HaV")
  file.rename(
    here::here("hav-fishery-figures.html"),
    file.path(output_path, "hav-fishery-figures.html")
  )
}
```
...
# 2025
# load(paste0(home, "/_mikaela/preCL_cleaned_20250314.RData"))

# 2026
load("/Users/maxlindmark/Data/atlas-data/preCL_cleaned_20260318.RData")
...
preCL <- preCL |> 
  mutate(omrade = NA,
         omrade = ifelse(SUB_REGION %in% c("27.3.c.22", "27.3.b.23", "27.3.d.24", "27.3.d.25", "27.3.d.26",
                         ...
                         "Östersjön", "Västerhavet"))

## test
# preCL |> filter(REDSKKOD %in% c(221, 312, 319, 323, 324, 330, 347) &
#                   omrade == "Östersjön") |> 
#   nrow()
...
# Fix the typos
cat <- cat |> 
  mutate(fishery = str_replace(fishery, "Dermersalt", "Demersalt"))
...
data_max_year <- max(year(as.Date(preCL$FISKEDATUM, format = "%Y%m%d")), na.rm = TRUE)
output_dir <- paste0(home, "/output/", data_max_year + 1, "/HaV")
...
asfis <- read_csv(paste0("/Users/maxlindmark/Data/atlas-data", "/ASFIS_sp_2021.csv")) 
swe_sp <- read_delim(paste0("/Users/maxlindmark/Data/atlas-data", "/latin_swedish_names.csv")) 
...
swe_sp_eel <- swe_sp |>
  slice_sample(n = 2) |>
  mutate(FNAMNSVE = "Ål",
         `Latinskt namn` = "Anguilla anguilla",
         `3A_CODE` = c("BB1", "GG1"))

swe_sp <- bind_rows(swe_sp_eel, swe_sp)

preCL <- preCL |>
  left_join(swe_sp, by = c("MAFKOD" = "3A_CODE"))

sort(unique(preCL$FNAMNSVE))
...
preCL |> filter(grepl("rocka", FNAMNSVE)) |> distinct(FNAMNSVE)

preCL <- preCL |> 
  mutate(FNAMNSVE = ifelse(FNAMNSVE %in% c("Blå havskatt", "Havskatt", "Havskatter"),
                           "Havskatt",
                           FNAMNSVE),
         FNAMNSVE = ifelse(FNAMNSVE %in% c("Taggmakrill", "Taggmakrillar- familj"),
                           "Taggmakrill",
                           FNAMNSVE),
         FNAMNSVE = ifelse(FNAMNSVE %in% c("Guldlax", "Guldlaxar (familj)"),
                           "Guldlax",
                           FNAMNSVE))

# NOTE: Herring in subdivisions with multiple stocks are assigned as follows:
...
preCL <- preCL |> 
  mutate(
    ...
    Bestånd = ifelse(FNAMNSVE == "Bleka / lyrtorsk" & SUB_REGION %in% c("27.4.a", "27.4.b", "27.4.c", "27.3.a.20", "27.3.a.21"),
                     "pol.27.3a4", Bestånd),
    ...
    Bestånd = ifelse(FNAMNSVE == "Hälleflundra", #FIXME: NOT IN STOCK DB! https://stockdatabase.ices.dk/default.aspx; MADE UP NAME
                     "halibut.27", Bestånd),
    Bestånd = ifelse(FNAMNSVE == "Knaggrocka" & SUB_REGION %in% c("27.4.a", "27.4.b", "27.4.c", "27.3.a.20", "27.3.a.21", "27.7.d"), 
                     "rjc.27.3a47d", Bestånd),
    Bestånd = ifelse(FNAMNSVE == "Knot" & SUB_REGION %in% c("27.4.a", "27.4.b", "27.4.c", "27.3.a.20", "27.3.a.21"), 
                     "gug.27.3a47d", Bestånd),
    Bestånd = ifelse(FNAMNSVE == "Klorocka" & SUB_REGION %in% c("27.4.a", "27.4.b", "27.4.c", "27.3.a.20", "27.3.a.21", "27.2.a.2"), 
                     "rjr.27.23a4", Bestånd),
    ...
    Bestånd = ifelse(FNAMNSVE == "Slätrocka" & SUB_REGION %in% c("27.4.a", "27.4.b", "27.4.c", "27.3.a.20", "27.3.a.21"), 
                     "rjb.27.3a4", Bestånd),
    ...
    Bestånd = ifelse(FNAMNSVE == "Nordhavsräka" & SUB_REGION %in% c("27.3.a.20", "27.3.a.21", "27.4.a"), #FIXME: 4a east? same as 27.4.a?
                     "pra.27.3a4a", Bestånd),
    # preCL |> filter(SUB_REGION %in% c("27.3.a.21", "27.3.c.22", "27.3.b.23", "27.3.d.24", "27.3.d.25", "27.3.d.26", "27.3.d.27", "27.3.d.28.2", "27.3.d.29", "27.3.d.30", "27.3.d.31", "27.3.d.32")) |> filter(FNAMNSVE == "Rödspätta") |> summarise(KG = sum(KG), .by = SUB_REGION)
    Bestånd = ifelse(FNAMNSVE == "Rödspätta" & SUB_REGION %in% c("27.3.a.21", "27.3.c.22", "27.3.b.23", "27.3.d.24", "27.3.d.25", "27.3.d.26", "27.3.d.27", "27.3.d.28.2", "27.3.d.29", "27.3.d.30", "27.3.d.31", "27.3.d.32"), 
                     "ple.27.21-32", Bestånd),
    Bestånd = ifelse(FNAMNSVE == "Rödspätta" & SUB_REGION %in% c("27.4.a", "27.4.b", "27.4.c", "27.3.a.20"), 
                     "ple.27.420", Bestånd),
    ...
    Bestånd = ifelse(FNAMNSVE == "Sill / strömming" & SUB_REGION %in% c("27.2.a.2", "27.2.a.1"),
    ...
    Bestånd = ifelse(FNAMNSVE == "Sjurygg" & SUB_REGION %in% c("27.3.d.24", "27.3.d.25", "27.3.d.26", "27.3.d.27", "27.3.d.28.1", "27.3.d.28.2", "27.3.d.29", "27.3.d.30", "27.3.d.31", "27.3.d.32"), #FIXME: Hav-list says 23 + 3a but there's no stock and catches are in other areas aswell
                     "lumpsucker.27.24-32", Bestånd),  #FIXME: NOT IN STOCK DB! https://stockdatabase.ices.dk/default.aspx; MADE UP NAME
    Bestånd = ifelse(FNAMNSVE == "Skrubbskädda" & SUB_REGION %in% c("27.4.a", "27.4.b", "27.4.c", "27.3.a.20"),
                     "fle.27.420", Bestånd),
    Bestånd = ifelse(FNAMNSVE == "Skrubbskädda" & SUB_REGION %in% c("27.3.a.21", "27.3.c.22", "27.3.b.23"),
                     "fle.27.2123", Bestånd),
    ...
    Bestånd = ifelse(FNAMNSVE == "Skarpsill" & SUB_REGION %in% c("27.3.a.20", "27.3.a.21", "27.4.a", "27.4.b", "27.4.c"), 
                     "spr.27.3a4", Bestånd),
    Bestånd = ifelse(FNAMNSVE == "Tobisfiskar" & SUB_REGION %in% c("27.3.a.20", "27.3.a.21", "27.3.b.22", "27.3.c.22"), 
                     "san.27.2022", Bestånd), #FIXME: 3a + 4 is not a stock, made up name
    Bestånd = ifelse(FNAMNSVE == "Tobisfiskar" & SUB_REGION %in% c("27.4.a", "27.4.b", "27.4.c"), 
...
preCL |> 
  filter(FNAMNSVE %in% c("Skoläst", "Siklöja", "Tobisfiskar", "Skarpsill", "Blåvitling", "Ål",
                         "Lax", "Öring", "Vitlinglyra", "Vitling", "Äkta tunga", "Torsk",
                         "Taggmakrill", "Slätvar", "Skrubbskädda", "Sill / strömming",
                         "Sandskädda", "Rödtunga", "Rödspätta", "Nordhavsräka", "Slätrocka",
                         "Piggvar", "Pigghaj", "Marulk", "Makrill", "Långa", "Kummel",
                         "Krabbtaska", "Kolja", "Knaggrocka", "Knot", "Hälleflundra", "Hummer", "Havskräfta",
                         "Havskatt", "Gråsej", "Fjärsing", "Bleka / lyrtorsk", "Bergtunga")) |> 
  summarise(n_stock = length(unique(Bestånd)), .by = FNAMNSVE) |> 
  arrange(FNAMNSVE)
```
...
```{r}
# Before summarising, we need to move some stocks that span both baltic and west coast to west coast

# These stocks cross the East-West border (SUB_REGION 27.3.c.22). Here we assign them to West coast fisheries
preCL <- preCL |> 
  mutate(fishery = ifelse(Bestånd %in% c("sol.27.20-24", "ple.27.21-32") &
                            grepl("Östersjön", fishery),
                          str_replace(fishery, "Östersjön", "Västerhavet"), 
                          fishery))

sort(unique(preCL$fishery))
sort(unique(preCL$Bestånd))
```
...
write_xlsx(preCL_sum, paste0(output_dir, "/table1.xlsx"))
```
